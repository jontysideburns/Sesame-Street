...
export default async function ComplianceEvidencePage() {
  ...
  return (
    <ComplianceShell
      ...
      title="Evidence"
      description="Evidence sits adjacent to the compliance workflow. It remains a standalone workspace, but it also belongs in the local compliance navigation because it completes the audit trail."
    >
        ...
        <article className="panel">
          ...
          <div className="stack">
            {evidence.reviewItems.map((item) => (
              <Link key={item.id} href="/compliance/review" className="document-card">
      ...
      <section className="panel">
        ...
        <div className="stack">
          {evidence.supersessions.map((item) => (
            <Link
              key={item.id}
              href={`/deals/${item.dealSlug}/periods/latest`}
              className="document-card"
            >
              <div className="status-row">
